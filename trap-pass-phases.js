(function () {
  ...
  async function loadPhases() {
    ...
    if (!response.ok) throw new Error("Trap Pass phases could not be loaded.");
  ...
  function phaseForPass(pass) {
    ...
    if (unlock >= 5) return { level: 6, title: "Metadata Ready" };
    if (unlock >= 4) return { level: 5, title: "Back Rooms" };
  ...
  function renderCurrentPhase(model) {
    ...
    mount.innerHTML = `
      <article class="pass-card">
        <span class="pass-stamp">Current Phase</span>
    ...
    mount.innerHTML = model.phases.map((phase) => {
      ...
      return `
        <article class="phase-card ${active ? "active" : "locked"}">
          ...
          <div class="meta-list compact">
            <div><span>Role</span><strong>${safe(phase.primary_role)}</strong></div>
            <div><span>Rooms</span><strong>${safe(phase.channels.join(", "))}</strong></div>
            <div><span>Unlock</span><strong>${safe(phase.unlock_signal)}</strong></div>
          </div>
          <div class="phase-routes"><span>Routes</span><strong>${safe(phase.site_routes.join(", "))}</strong></div>
        </article>
      `;
    }).join("");
  }
  ...
  function renderMetadata() {
    const mount = document.querySelector("[data-metadata-preview]");
    if (!mount) return;
    const pass = window.TrapHouse?.getCurrentPass ? window.TrapHouse.getCurrentPass() : null;
    if (!pass || !window.TrapHouse?.getTrapPassMetadata) {
      mount.textContent = "Claim or select a Trap Pass to view the shareable pass summary.";
      return;
    }
    mount.textContent = JSON.stringify(window.TrapHouse.getTrapPassMetadata(pass), null, 2);
  }
  ...
  async function initTrapPassPhases() {
    try {
      ...
      document.querySelectorAll("[data-phase-error]").forEach((mount) => {
        ...
        mount.innerHTML = `<strong>Phase load error.</strong><p>${safe(error.message)}</p>`;
      });
    }
  }
